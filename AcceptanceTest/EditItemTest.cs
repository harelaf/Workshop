...
namespace AcceptanceTest
{
    ...
    public class EditItemTest
    {
        ...
        {
            Response response_reg = marketAPI.EditItemName(registered_VisitorToken, storeName_inSystem, itemID_inStock_1, "Eshel");
            if (response_reg.ErrorOccured)
                Assert.Fail(response_reg.ErrorMessage);
            List<ItemDTO> res = marketAPI.GetItemInformation(registered_VisitorToken, "Eshel", "", "").Value;
            if(res == null || res.Count == 0)
            {
                Assert.Fail("Item Eshel supposed to be found");
            }
            marketAPI.EditItemName(registered_VisitorToken, storeName_inSystem, itemID_inStock_1, "Leben");
            res = marketAPI.GetItemInformation(registered_VisitorToken, "Leben", "", "").Value;
            if (res == null || res.Count == 0)
            {
        ...
        {
            Response response_reg = marketAPI.EditItemName(registered_VisitorToken, storeName_inSystem, itemID_inStock_2, "apple juice");
            if (!response_reg.ErrorOccured)
                Assert.Fail("should've faild: item isn't in stock");
        }

        [TestMethod]
        public void TestEditItemName_Sad_VisitorWithoutPermition()
        {
            Response response_guest = marketAPI.EditItemName(guest_VisitorToken, storeName_inSystem, itemID_inStock_1, "Eshel");
            if (!response_guest.ErrorOccured)
                Assert.Fail("should've faild: Visitor doesn't have a permition to edit item");
        }
        ...
        {
            Response response_reg = marketAPI.EditItemPrice(registered_VisitorToken, storeName_inSystem, itemID_inStock_2, 12.5);
            if (!response_reg.ErrorOccured)
                Assert.Fail("should've faild: item isn't in stock");
        }

        [TestMethod]
        public void TestEditItemPrice_Sad_VisitorWithoutPermition()
        {
            Response response_guest = marketAPI.EditItemPrice(guest_VisitorToken, storeName_inSystem, itemID_inStock_1, 12.5);
            if (!response_guest.ErrorOccured)
                Assert.Fail("should've faild: Visitor doesn't have a permition to edit item");
        }
        ...
        {
            Response response_reg = marketAPI.EditItemDescription(registered_VisitorToken, storeName_inSystem, itemID_inStock_2, RandomString(50));
            if (!response_reg.ErrorOccured)
                Assert.Fail("should've faild: item isn't in stock");
        }

        [TestMethod]
        public void TestEditItemDescription_Sad_VisitorWithoutPermition()
        {
            Response response_guest = marketAPI.EditItemDescription(registered_VisitorToken, storeName_inSystem, itemID_inStock_2, RandomString(50));
            if (!response_guest.ErrorOccured)
                Assert.Fail("should've faild: Visitor doesn't have a permition to edit item");
        }
