...
namespace MarkrtTesting
{
    ...
    public class UnitTestPurchase
    {
        ...
        public void setUp()
        {
            ...
            for (int i = 1; i < 10; i++)
            {
                Mock<Item> itemMoq = new Mock<Item>(i, "item_"+i, 1.5, "");
        ...
        public void TestPurchase_PaymentNShippingAproved()
        {
            ...
            shippingProxyMoq.Setup(x => x.ShippingApproval(It.IsAny<string>())).Returns(true);
            //action
            try{
                purchase.Purchase("adr", cartMoq.Object);
            }
            catch (Exception ex) { NUnit.Framework.Assert.Fail(ex.Message); }
        ...
        public void TestPurchase_PaymentAproved_ShippingNot()
        {
            ...
            setUp();
            paymentProxyMoq.Setup(x => x.Pay(It.IsAny<double>())).Returns(true);
            shippingProxyMoq.Setup(x => x.ShippingApproval(It.IsAny<string>())).Returns(false);
            //action
            try
            {
                purchase.Purchase("adr", cartMoq.Object);
