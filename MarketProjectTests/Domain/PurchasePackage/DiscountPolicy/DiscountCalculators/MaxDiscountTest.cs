...
namespace MarketProjectTests.Domain.PurchasePackage.DiscountPolicy.DiscountCalculators
{
    ...
    public class MaxDiscountTest
    {
		...
		public void GetTotalDiscount_noCondition_success()
		{
			...
			disList.Add(dis1);
			disList.Add(dis2);
			MaxDiscount maxDis = new MaxDiscount(disList);
			store.AddDiscount(maxDis);
			basket.AddItem(item, amount);

			//act
			double expected = dis2.GetTotalDiscount(basket);
			double actual = maxDis.GetTotalDiscount(basket);

			//Assert
			Assert.AreEqual(amount * price * percents_to_subtract / 100, expected);
		...
		public void GetTotalDiscount_withCondition_success()
		{
			...
			//Assert
			Assert.AreEqual(basket.GetTotalPrice() * percents_to_subtract / 100, expected);
			Assert.AreEqual(expected, actual);
		}
		[TestMethod]
		public void GetTotalDiscount2_withCondition_success()
		{
			double amount_to_subtract = 2;
			double percents_to_subtract = 30;
			int amount = 10;
		...
		public void GetTotalDiscount_withCondition_fail()
		{
			...
			disList.Add(dis1);
			disList.Add(dis2);
			MaxDiscount maxDis = new MaxDiscount(disList, andCondition);
			store.AddDiscount(maxDis);
			basket.AddItem(item, amount);
			basket.AddItem(item2, 20);
